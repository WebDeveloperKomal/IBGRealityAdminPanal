...
function populateTable(data) {
    ...
    data.forEach((item, index) => {
        ...
        row.innerHTML = `
            ...
            <td>
                <img src="http://localhost:8080/auth/get-image/${item.id}" alt="Image" class="img-icon" data-id="${item.id}" style="width: 50px; height: 50px; cursor: pointer;">
            </td>
            <td>
...
function handleImageClick(id) {
    const imageUrl = `http://localhost:8080/auth/get-image/${id}`;
    window.open(imageUrl, '_blank');
}


function handleUpdateClick(id, name, designation) {
    const updateUrl = `update-our-team.html?id=${id}&name=${encodeURIComponent(name)}&designation=${encodeURIComponent(designation)}`;
